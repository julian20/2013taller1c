...
MousePosition MapCameraController::getMousePosition() {
	...
	SDL_GetMouseState(&mouseX, &mouseY);
	if (mouseAtLeft() && mouseAtTop())
		return MOUSE_TOP_LEFT;
	if (mouseAtRight() && mouseAtTop())
		return MOUSE_TOP_RIGHT;
	if (mouseAtLeft() && mouseAtBottom())
		return MOUSE_BOTTOM_LEFT;
	if (mouseAtRight() && mouseAtBottom())
		return MOUSE_BOTTOM_RIGHT;
	if (mouseAtLeft())
		return MOUSE_LEFT;
	if (mouseAtRight())
		return MOUSE_RIGHT;
...
int MapCameraController::computeMovementSpeed(MousePosition pos) {
	...
	case MOUSE_TOP_RIGHT:
		speed = (movementMargin - mouseY) / 4
				+ (movementMargin + (mouseX - screen->w)) / 4;
		break;
	case MOUSE_BOTTOM_LEFT:
		speed = (movementMargin + (mouseY - screen->h)) / 4
				+ (movementMargin - mouseX) / 4;
		break;
